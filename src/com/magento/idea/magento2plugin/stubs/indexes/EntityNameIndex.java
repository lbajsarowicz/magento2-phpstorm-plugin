 ...
 */
public class EntityNameIndex extends ScalarIndexExtension<String> {
    public static final ID<String, Void> KEY = ID.create("com.magento.idea.magento2plugin.stubs.indexes.entity_name");
    ...
    public DataIndexer<String, Void, FileContent> getIndexer() {
        return inputData -> {
            ...
            if (psiFile instanceof PhpFile) {
                grabEntityNamesFromPhpFile((PhpFile) psiFile, map);
            } else if (psiFile instanceof XmlFile) {
                grabEntityNamesFromXmlFile((XmlFile) psiFile, map);
            }
    ...
    private void grabEntityNamesFromPhpFile(PhpFile file, Map<String, Void> map) {
    ...
    private void recursiveFill(List<String> results, PsiElement psiElement) {
        if (PhpPatternsHelper.STRING_METHOD_ARGUMENT.accepts(psiElement) && isInContextOfDispatchMethod(psiElement)) {
            String entityName= StringUtil.unquoteString(psiElement.getText());
    ...
    private void grabEntityNamesFromXmlFile(XmlFile file, Map<String, Void> map) {
        ...
        if (xmlDocument != null) {
            ...
            if (xmlRootTag != null) {
                for (XmlTag entityTag : xmlRootTag.findSubTags("entity")) {
                    String name = entityTag.getAttributeValue("name");
    ...
    public FileBasedIndex.InputFilter getInputFilter() {
        return file -> (
                file.getFileType() == PhpFileType.INSTANCE
                    || (file.getFileType() == XmlFileType.INSTANCE && file.getName().endsWith("Data.xml"))
        );
    }
