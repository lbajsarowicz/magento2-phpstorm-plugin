...
public class TestNameIndex extends ScalarIndexExtension<String> {
    public static final ID<String, Void> KEY = ID.create("com.magento.idea.magento2plugin.stubs.indexes.test_name");
    ...
    public DataIndexer<String, Void, FileContent> getIndexer() {
        return inputData -> {
            ...
            if (psiFile instanceof PhpFile) {
                grabTestNamesFromPhpFile((PhpFile) psiFile, map);
            } else if (psiFile instanceof XmlFile) {
                grabTestNamesFromXmlFile((XmlFile) psiFile, map);
            }
    ...
    private void grabTestNamesFromPhpFile(PhpFile file, Map<String, Void> map) {
    ...
    private void recursiveFill(List<String> results, PsiElement psiElement) {
        if (PhpPatternsHelper.STRING_METHOD_ARGUMENT.accepts(psiElement) && isInContextOfDispatchMethod(psiElement)) {
            String testName= StringUtil.unquoteString(psiElement.getText());
    ...
    private void grabTestNamesFromXmlFile(XmlFile file, Map<String, Void> map) {
        XmlDocument xmlDocument = file.getDocument();
        if (xmlDocument != null) {
            XmlTag xmlRootTag = xmlDocument.getRootTag();
            if (xmlRootTag != null) {
                for (XmlTag testTag : xmlRootTag.findSubTags("test")) {
                    String name = testTag.getAttributeValue("name");
    ...
    public FileBasedIndex.InputFilter getInputFilter() {
        return file -> (
                file.getFileType() == PhpFileType.INSTANCE
                    || (file.getFileType() == XmlFileType.INSTANCE && file.getName().endsWith("Test.xml"))
        );
    }
