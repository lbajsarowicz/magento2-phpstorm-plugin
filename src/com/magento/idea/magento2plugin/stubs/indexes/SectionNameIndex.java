 ...
 */
public class SectionNameIndex extends ScalarIndexExtension<String> {
    public static final ID<String, Void> KEY = ID.create("com.magento.idea.magento2plugin.stubs.indexes.section_name");
    ...
    public DataIndexer<String, Void, FileContent> getIndexer() {
        return inputData -> {
            ...
            if (psiFile instanceof PhpFile) {
                grabSectionNamesFromPhpFile((PhpFile) psiFile, map);
            } else if (psiFile instanceof XmlFile) {
                grabSectionNamesFromXmlFile((XmlFile) psiFile, map);
            }
    ...
    private void grabSectionNamesFromPhpFile(PhpFile file, Map<String, Void> map) {
    ...
    private void recursiveFill(List<String> results, PsiElement psiElement) {
        if (PhpPatternsHelper.STRING_METHOD_ARGUMENT.accepts(psiElement) && isInContextOfDispatchMethod(psiElement)) {
            String sectionName= StringUtil.unquoteString(psiElement.getText());
    ...
    private void grabSectionNamesFromXmlFile(XmlFile file, Map<String, Void> map) {
        ...
        if (xmlDocument != null) {
            ...
            if (xmlRootTag != null) {
                for (XmlTag sectionTag : xmlRootTag.findSubTags("section")) {
                    String name = sectionTag.getAttributeValue("name");
    ...
    public FileBasedIndex.InputFilter getInputFilter() {
        return file -> (
                file.getFileType() == PhpFileType.INSTANCE
                    || (file.getFileType() == XmlFileType.INSTANCE && file.getName().endsWith("Section.xml"))
        );
    }
