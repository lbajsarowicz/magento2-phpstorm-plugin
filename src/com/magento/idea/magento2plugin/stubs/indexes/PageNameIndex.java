 ...
 */
public class PageNameIndex extends ScalarIndexExtension<String> {
    public static final ID<String, Void> KEY = ID.create("com.magento.idea.magento2plugin.stubs.indexes.page_name");
    ...
    public DataIndexer<String, Void, FileContent> getIndexer() {
        return inputData -> {
            ...
            if (psiFile instanceof PhpFile) {
                grabPageNamesFromPhpFile((PhpFile) psiFile, map);
            } else if (psiFile instanceof XmlFile) {
                grabPageNamesFromXmlFile((XmlFile) psiFile, map);
            }
    ...
    private void grabPageNamesFromPhpFile(PhpFile file, Map<String, Void> map) {
    ...
    private void recursiveFill(List<String> results, PsiElement psiElement) {
        if (PhpPatternsHelper.STRING_METHOD_ARGUMENT.accepts(psiElement) && isInContextOfDispatchMethod(psiElement)) {
            String pageName= StringUtil.unquoteString(psiElement.getText());
    ...
    private void grabPageNamesFromXmlFile(XmlFile file, Map<String, Void> map) {
    ...
    public FileBasedIndex.InputFilter getInputFilter() {
        return file -> (
                file.getFileType() == PhpFileType.INSTANCE
                    || (file.getFileType() == XmlFileType.INSTANCE && file.getName().endsWith("Page.xml"))
        );
    }
